...
alter table commande add numeroBonCommande bigint;


-- creation of table livraison
create table livraison(
	idLivraison bigint identity primary key,
	idCommande bigint,
	dateLivraison date,
	numeroBonLivraison bigint,
	constraint fkLivraisonCommande foreign key(idLivraison) references commande(idCommande)
)
...
-- alter table detailCommande add nbPieceLivré bigint !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!????????????????????????????????????????????????????????????????????????????
---------   
---------
